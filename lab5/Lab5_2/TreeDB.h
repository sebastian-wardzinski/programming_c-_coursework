...
private:
   ...
   bool insertHelper(TreeNode *node, TreeNode *root);
   DBentry* findHelper (TreeNode *root, string name);
   bool removeHelper(TreeNode* parent, TreeNode* node, string name);
   void clearHelper(TreeNode* node);
   void countActiveHelper(TreeNode* node ,int &count) const;
